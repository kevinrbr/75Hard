...
export function buildCalendar(challenge: Challenge, dailyLogs: DailyLog[]): CalendarDay[] {
...
export function computeBestStreak(calendar: CalendarDay[]): number {
  let best = 0
  let current = 0
...
export function computeGoalPerformance(
  goals: Goal[],
...
export function computeWeeklyHistory(dailyLogs: DailyLog[]): { weekLabel: string; completionRate: number }[] {
...
export function computeStats(
  ...
  const dailyCompletionRate =
    dailyCompletions.length === 0 ? 0 : Math.round((dailyDone / dailyCompletions.length) * 100)
  const weeklyCompletionRate =
    weeklyCompletions.length === 0 ? 0 : Math.round((weeklyDone / weeklyCompletions.length) * 100)
