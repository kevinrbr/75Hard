...
import { loadInitialState, saveState } from '@/services/challengeService';
import { diffInDays, startOfWeek, todayISO } from '@/utils/date';
...
export const useChallengeStore = defineStore('challenge', () => {
    ...
    const todayDailyCompletions = computed(() => completions.value.filter((completion) => completion.type === 'daily' && completion.completionDate === today.value));
    const weekCompletions = computed(() => completions.value.filter((completion) => completion.type === 'weekly' && completion.completionDate === weekStart.value));
    const dailyCompletionCount = computed(() => todayDailyCompletions.value.filter((completion) => completion.completed).length);
    const dailyProgress = computed(() => {
        if (dailyGoals.value.length === 0)
            return 0;
        return Math.round((dailyCompletionCount.value / dailyGoals.value.length) * 100);
    });
    const weeklyCompletionCount = computed(() => weekCompletions.value.filter((completion) => completion.completed).length);
    const weeklyProgress = computed(() => {
        if (weeklyGoals.value.length === 0)
            return 0;
    ...
    const stats = computed(() => {
        ...
        return computeStats(challenge.value, goals.value, completions.value, dailyLogs.value, weeklyLogs.value);
    });
    const weeklyTrend = computed(() => computeWeeklyHistory(dailyLogs.value));
    function completionFor(goalId, date, type) {
        return completions.value.find((completion) => completion.goalId === goalId &&
            completion.completionDate === date &&
            completion.type === type);
    }
    function ensureCompletion(goalId, date, type) {
    ...
    async function persist() {
        if (!challenge.value || !settings.value)
            return;
        const payload = {
    ...
    async function setStartDate(startDate) {
        if (!challenge.value)
            return;
    ...
    async function toggleDailyTask(goalId) {
    ...
    async function toggleWeeklyTask(goalId) {
